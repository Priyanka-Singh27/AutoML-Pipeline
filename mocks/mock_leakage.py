from .mock_generator import make_classification_mock, add_leakage_feature

mock_df = make_classification_mock(n_samples=5000)
# inject a feature that basically is the target
mock_df = add_leakage_feature(mock_df, target_col='target', noise=0.01)
...
mock_audit = {
    ...
    'missing': {}, 
    # Notice leakage_col is not in drop_candidates - user forced keep
    'drop_candidates': {}, 
    'high_cardinality': [],
    
    'high_correlations': [], 
    'leakage_candidates': {'leakage_col': {'score': 0.99}}, # <--- flagged
    'feature_target_correlation': {
        'leakage_col': {'score': 0.99, 'signal': 'strong'},
